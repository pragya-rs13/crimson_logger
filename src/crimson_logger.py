...
class CrimsonLogger:
    ...
    def __init__(self, 
                 config: CrimsonLogConfig, 
                 writer: CrimsonWriter, 
                 formatter: Formatter):
        self._config = config
        self._writer = writer
        ...
        self._main_thread = (
            threading.current_thread() if self._config.thread_model == ThreadModel.SINGLE else None
        )
    ...
    def write_log(self, log_level: LogLevel, content: str, namespace: str):
        ...
        if(log_level >= self._log_level):
            ...
            if self._config.thread_model == ThreadModel.SINGLE:
                ...
                if self._main_thread and threading.current_thread() != self._main_thread:
...
class CrimsonLoggerBuilder:
    ...
    def set_sink(self):
        ...
        if self._config.sink_type == "FILE":
            ...
            raise ConfigException("[ERROR] Configuration is not for file type sink \n" +
    ...
    def set_writer(self):
        ...
        if self._config.write_mode == WriteMode.SYNC:
            ...
            if self._config.thread_model == ThreadModel.MULTI:
                print("[WARN] thread_model MULTI with Sync Writer may block calling threads")
            
        elif self._config.write_mode == WriteMode.ASYNC:
            self._writer = AsyncWriter().set_sink(self._sink)
            self._writer.start()
            if self._config.thread_model == ThreadModel.SINGLE:
                print("[WARN] thread_model SINGLE with Async Writer may delay logs until close() is called")
        else:
            raise ConfigException("[ERROR] Unknown writer type passed, please use `with_custom_writer` while building")
    ...
    def set_formatter(self):
        ...
        """
        if not self._config:
            raise ConfigException("[ERROR] Config init is the first step, please use `with_config(__config_path))` method first")
    ...
    def build(self):
        ...
        # validation
        if not all([self._config, self._formatter, self._sink, self._writer, self._name]):
            raise ConfigException("LoggerBuilder not fully initialized. Missing Attributes")
